...
(function(exports) {
    ...
    var meta_entry = (function(){
        return function(){
            ...
            this.update_dependence_addressing = function(filename){
                ...
                if( filename.substring(0,1) == "/" && filename.substring(0,base_dir.length) == base_dir ){
                    filename = filename.substring(base_dir.length+1)
                }else{
                    filename = path.resolve(base_dir+filename).substring(base_dir.length)
                }
                return filename
            }
    ...
    var meta_manager = function(base_dir){
        ...
        this.load = function(meta_path){
            d = fs.readFileSync(meta_path, 'utf8')
            var entry = new meta_entry()
