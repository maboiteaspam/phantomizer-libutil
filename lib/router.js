...
//    ]
    ...
    this.load = function(then){
      if( this.has_loaded || routes.length == 0 ){
        then();
      }else{
        var n = 0;
        var that = this;
    ...
    this.match = function(absolute_url){
      if(!this.has_loaded){
        throw ('attempting to use a non initialized router');
      }
      for( var n in routes ){
        var route = routes[n];
        if(route.urls &&
  ...
  function read_url(route,then){
    ...
    request(url, options, function (error, response, body) {
      if( error ){
        ...
        if( body.match(/^([a-z0-9]+\s)/i) && body.match(/\S+\s+[a-z0-9]+\s+$/i) ){
          body = body.replace(/^([a-z0-9]+\s)/i,"")
          body = body.replace(/(\S+)\s+[a-z0-9]+\s+$/i,"$1")
        }
        then(true,body);
      }
    })
