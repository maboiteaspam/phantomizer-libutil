...
(function(exports) {
	...
	var html_utils = function(){
            ...
            for( var n in nodes ){
                ...
                if( src != ""){
                    ...
                    if( !has_domain && asrc.substring(0,1) != "/" ){
                        asrc = path.normalize(base_url+asrc);
                    }
                    retour.push({
                        'node':nodes[n],
		...
		this.find_optimized_nodes = function(html_content, requirejs_src, base_url){
            var retour = []

            var nodes = get_script_nodes(html_content)
            for( var n in nodes ){
                var optimized = get_attr("optimized", nodes[n]); // require js script src
                ...
                if( optimized != "" ){
                    ...
                    if( !has_domain && asrc.substring(0,1) != "/" ){
                        asrc = path.normalize(base_url+asrc).substr(1);
                    }
